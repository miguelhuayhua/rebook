...
export default function ProductDetailPage({ producto }: Props) {
  ...
  // Función para manejar el cambio de imagen desde las thumbnails
  const handleImageThumbnailClick = (imageUrl: string) => {
    setCurrentImageUrl(imageUrl)
  }

  // Inicialización única del producto
  useEffect(() => {
    if (producto && !isInitialized) {
      let initialImageUrl = "/placeholder.svg"
      
      if (producto.variantes.length > 0) {
        const defaultVariant = producto.variantes[0]
        setSelectedVariant(defaultVariant)
        initialImageUrl = defaultVariant.imagen?.url || producto.imagenes[0]?.url || "/placeholder.svg"
      } else if (producto.imagenes.length > 0) {
        initialImageUrl = producto.imagenes[0].url
      }
      
      setCurrentImageUrl(initialImageUrl)
      setIsInitialized(true)
    }
  }, [producto, isInitialized])
              ...
              {producto.imagenes.length > 1 && (
                <div className="flex space-x-3 justify-center">
                  {producto.imagenes.map((imagen, index) => (
                    <button
                      key={imagen.id}
                      onClick={() => handleImageThumbnailClick(imagen.url)}
            ...
            <div className="space-y-8 fade-in">
              ...
              <div className="grid grid-cols-3 gap-6 pt-8 border-t border-gray-200/50">
                ...
                <div className="text-center group">
                  ...
                  <p className="text-xs text-gray-500">Entrega asegurada</p>
                </div>
                <div className="text-center group">
                  ...
                  <p className="text-xs text-gray-500">Auténticos</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <Tabs defaultValue="especificaciones" className="w-full max-w-4xl mx-auto">
                <TabsList className="grid w-full grid-cols-2 bg-gray-100/50 backdrop-blur-sm h-12">
                  {producto.descripcion && producto.descripcion.trim() && (
                    <TabsTrigger
                      value="descripcion"
                      className="font-medium  data-[state=active]:shadow-sm"
                    >
